...
/**
    ...
    public function updateRequestStatus($requestId, $status)
    {
        ...
        // E-posta gönderebilmek için önce isteğin ve kullanıcının mevcut bilgilerini alıyoruz.
        $request = $requestModel->getRequestWithUserDetails($requestId);
        
        // Veritabanında durumu güncelliyoruz.
        $success = $requestModel->updateStatus($requestId, $status);

        // Güncelleme başarılıysa ve kullanıcı bilgileri alınabildiyse e-posta gönder.
        if ($success && $request) {
            
            $mailService = new \App\Services\MailService();
            $templateName = null;

            // Duruma göre doğru e-posta şablonunu seçiyoruz.
            if ($status === 'completed') {
                $templateName = 'request_approved'; // Onaylandı e-postası
            } elseif ($status === 'rejected') {
                $templateName = 'request_rejected'; // Reddedildi e-postası
            }

            // Eğer gönderilecek bir şablon varsa
            if ($templateName) {
                $context = [
                ...
                // E-postayı gönder
                $mailService->sendTemplateEmail($templateName, $request['user_email'], $context);
            }
        }

        // Admini tekrar istekler listesine yönlendir.
    ...
    public function deleteRequest($requestId) {
        $requestModel = new \App\Models\RequestModel();
        // Hatalı olan 'deleteById' çağrısını 'delete' olarak düzeltiyoruz.
